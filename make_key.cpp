...
{
	constexpr uint32 mac_key[] = {1, 2, 3, 4}; // not required to be secret
	CbcMac mac(mac_key);
	uint32 buffer[CbcMac::stateSize] = {};
	for (auto it = password.begin() ; it != password.end(); )
	{
		...
		// use a simple scheme: one uint32 per character of the password
